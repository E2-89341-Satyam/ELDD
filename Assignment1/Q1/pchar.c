...
static long pchar_ioctl(struct file *pfile,unsigned int cmd,unsigned long param)
{
    ...
    switch(cmd){
        case FIFO_CLEAR:
            pr_info("%s:pchar_ioctl() - cleared fifo\n",THIS_MODULE->name);
            kfifo_reset(&dev->buffer);
            break;
        case FIFO_GET_INFO:
            info.size = kfifo_size(&dev->buffer);
            info.len = kfifo_len(&dev->buffer);
            info.avail = kfifo_avail(&dev->buffer);
            copy_to_user((void *)param,&info,sizeof(fifo_info_t));
            pr_info("%s:pchar_ioctl() -kfifo info:size=%d,len=%d,avail=%d\n",THIS_MODULE->name,info.size,info.len,info.avail);
            break;

        case FIFO_RESIZE:
            int ret;
            unsigned char *temp_buf;
            // allocte temp memory (using kmalloc()) to hold old buffer contents
            temp_buf = kmalloc(sizeof(&dev->buffer),GFP_KERNEL);
            // read fifo contents into temp memory (using kfifo_out())
            ret = kfifo_out(&dev->buffer,temp_buf,sizeof(&dev->buffer));
            // free the old buffer (using kfifo_free())
            kfifo_free(&dev->buffer);
            // get size from 3rd arg (i.e. param)

            // allocate new buffer with given size (using kfifo_alloc())
            ret = kfifo_alloc(&dev->buffer,param,GFP_KERNEL);
            // copy contents from temp memory to new buffer (using kfifo_in())
            ret = kfifo_in(&dev->buffer,temp_buf,sizeof(&dev->buffer));
            // release temp memory (using kfree())
            kfree(temp_buf);
            break;

        default :
        pr_err("%s:invalid cmd in ochar_ioctl()\n",THIS_MODULE->name);
        return -EINVAL;
